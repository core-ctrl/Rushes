...
function generateWhyReasons(item, profile) {
  const reasons = [];
...
function normalizeProviderBuckets(providers = null) {
  return {
...
function getWatchProviders(providers = null, mode = "all") {
...
function getContentStatus(item = {}, providers = item.availability || item.providers || null, today = new Date()) {
...
function buildRecommendationReasons(item = {}, profile = {}) {
  const reasons = [];
  const providers = getWatchProviders(item.availability || item.providers);
...
function withDecisionMetadata(item = {}, options = {}) {
...
function matchesOttFilter(item = {}, filters = {}) {
