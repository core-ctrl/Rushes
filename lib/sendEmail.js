              ...
              <tr>
                <td style="background:#0f0f0f;padding:24px 40px;text-align:center;border-top:1px solid rgba(255,255,255,0.05);">
                  <p style="margin:0 0 4px;color:rgba(255,255,255,0.2);font-size:11px;line-height:1.7;">
                    Sent by MovieFinder · <a href="https://moviefinderforyou.com" style="color:#E63946;text-decoration:none;">moviefinderforyou.com</a>
                  </p>
...
export async function sendVerificationEmail(email, token, username = "there") {
  const verifyUrl = `${appUrl()}/verify-email?token=${token}`;

  const content = `
    <h2 style="${headingStyle}">Verify your email</h2>
    <p style="${bodyTextStyle}">
      Hey <strong style="color:#fff;">@${username}</strong>, one click and you're in.
      Confirm your email to start discovering cinema that actually moves you.
    </p>
    ...
    <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:10px;padding:14px 16px;">
      <p style="margin:0;color:rgba(255,255,255,0.3);font-size:11px;text-align:center;word-break:break-all;">
        Or copy this link: <span style="color:#E63946;">${verifyUrl}</span>
      </p>
    </div>
    <p style="margin:16px 0 0;color:rgba(255,255,255,0.25);font-size:12px;text-align:center;">
...
export async function sendWelcomeEmail(email, username = "there") {
  const content = `
...
export async function sendOTPEmail(email, username = "there") {
...
export async function sendLoginThankYouEmail(email, username = "there") {
  const content = `
...
export async function sendDecisionAlertEmail(email, username = "there", message = "", subject = "MovieFinder alert") {
  const content = `
