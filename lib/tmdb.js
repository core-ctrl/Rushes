...
export async function fetchByGenre(genreId, page = 1, type = "movie", sortBy = "popularity.desc") {
  ...
  const d = await req(
    ...
    p({
      ...
      sort_by: sortBy,
      page,
      "vote_count.gte": sortBy.includes("vote_average") ? 300 : 0
    })
  );
...
export async function discoverContent(params = {}, type = "movie") {
  const endpoint = type === "tv" ? "discover/tv" : "discover/movie";
